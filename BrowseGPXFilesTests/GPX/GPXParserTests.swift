...
import Foundation
import Testing
@testable import BrowseGPXFiles
...
struct GPXParserTests {
    ...
    static let gpx: GPX = {
        let url = TestBundle.bundle.resourceURL!.appending(path: "GPXTest/manual.gpx")
    ...
    @Test func testNoContent() throws {
        #expect(throws: XMLError.parsingError(0)) {
            let url = TestBundle.bundle.resourceURL!.appending(path: "GPXTest/no-content.gpx")
    ...
    @Test func testNoTrack() throws {
        let url = TestBundle.bundle.resourceURL!.appending(path: "GPXTest/no-track.gpx")
